          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }} className="w-full max-w-md rounded-lg border border-[#E0E0E0] shadow-[0_2px_10px_rgba(0,0,0,0.1)] bg-white">
            <motion.form onSubmit={onSubmit} className="p-6 space-y-4" initial="hidden" animate="show" variants={{ hidden: {}, show: { transition: { staggerChildren: 0.08 } } }}>
              ...
              <motion.div variants={{ hidden: { opacity: 0, y: 10 }, show: { opacity: 1, y: 0 } }}>
                <label>{t('email')}</label>
                <input className="input mt-1" type="email" placeholder={t('email_placeholder')} value={email} onChange={(e)=>setEmail(e.target.value)} required />
              </motion.div>
              <motion.div variants={{ hidden: { opacity: 0, y: 10 }, show: { opacity: 1, y: 0 } }}>
              ...
              <motion.div className="flex items-center justify-between" variants={{ hidden: { opacity: 0, y: 10 }, show: { opacity: 1, y: 0 } }}>
                <label className="inline-flex items-center gap-2 text-sm text-[#0078D4]"><input type="checkbox" className="rounded" /> {t('remember_me')}</label>
                <Link href="#" className="text-sm text-[#0078D4] hover:underline">{t('forgot_password')}</Link>
              </motion.div>
              <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.99 }} type="submit" className="w-full rounded-xl bg-[#00A9E0] text-white py-3 shadow-[0_8px_24px_rgba(0,0,0,0.2)]" disabled={loading}>{loading ? t('signing_in') : t('login')}</motion.button>
              <motion.div className="text-sm text-[#0078D4] text-center" variants={{ hidden: { opacity: 0, y: 10 }, show: { opacity: 1, y: 0 } }}>{t('no_account')} <Link className="hover:underline" href="/auth/signup">{t('sign_up')}</Link></motion.div>
            </motion.form>
          </motion.div>
        </section>
